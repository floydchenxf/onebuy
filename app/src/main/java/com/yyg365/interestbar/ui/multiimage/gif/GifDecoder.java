package com.yyg365.interestbar.ui.multiimage.gif;
...
public class GifDecoder {
	...
	private Bitmap getFrame(int n) {
		Bitmap im = null;
		if ((n >= 0) && (n < frameCount)) {
			im = ((GifFrame) frames.get(n)).getImage();
		}
		return im;
	}
...
//		long time = System.currentTimeMillis();
	...
	private void readImage() {
		...
		image = Bitmap.createBitmap(width, height, Config.ALPHA_8);
		// createImage(width, height);
		setPixels(); // transfer pixel data to image
		GifFrame frame = new GifFrame(image, delay);
		frames.add(frame); // add image to frame
